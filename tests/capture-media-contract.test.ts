...
test('web capture setup points users at the manifest-owning extension folder', () => {
  const captureSettings = read('macos-app/Loom/Sources/CaptureSettingsView.swift');
  const captureHelp = read('macos-app/Loom/Sources/CaptureHelpView.swift');

  assert.match(captureSettings, /private var extensionResourcesPath: String/);
  assert.match(captureSettings, /LoomWebExtension\.appex/);
  assert.match(captureSettings, /appendingPathComponent\("Resources"\)/);
  assert.match(captureSettings, /manifest\.json/);
  assert.match(captureSettings, /Section\("Browser Extension"\)/);
  assert.match(captureSettings, /Do not choose the parent LoomWebExtension folder/);
  assert.match(captureHelp, /The bookmarklet is a fallback, not the rich-media path/);
});
