...
test('reader embeds snapshot-backed visual blocks only when per-resource anchors exist', () => {
  ...
  // 2026-05-02 click-to-load interactive snapshot embed. Static
  // composite-media / structured-visual screenshots are wrapped in
  // a button by default; clicking mounts a sandboxed iframe at the
  // snapshot route's targeted-embed mode (?embed=1&target=...) so
  // the source page's scroll-linked diagrams (e.g. flipdisc.io's
  // "Aluminum Frame / Controller Board / Power Connection / Data
  // Connection" 4-state widget) run live. The legacy targeted-frame
  // mode that auto-mounted at page load was retired because it ate
  // wheel events and dispatched parent scroll commands; click-to-
  // load means at most one iframe is active, only on user intent.
  assert.match(source, /data-loom-interactive-snapshot-mode="click-to-load"/);
  assert.match(source, /class="loom-snapshot-load"/);
  assert.match(source, /data-loom-snapshot-href=/);
  // The mounted iframe sandbox drops allow-same-origin so the
  // captured-page JS cannot reach parent cookies / top.location.
  assert.match(source, /'allow-scripts allow-forms allow-popups allow-popups-to-escape-sandbox allow-presentation'/);
  ...
  assert.match(source, /const figureBody = liveHref/);
...
test('AST snapshot-backed visual blocks mount targeted interactive snapshots on click', () => {
  const source = read('app/loom-render/capture/page.tsx');
  const start = source.indexOf('function AstImageBlock');
  const end = source.indexOf('function AstVideoBlock');
  assert.ok(start > 0 && end > start, 'AstImageBlock must exist before AstVideoBlock');
  const astImageBlock = source.slice(start, end);

  assert.match(astImageBlock, /SNAPSHOT_BACKED_KINDS\.includes\(block\.mediaRole \|\| ''\)/);
  assert.match(astImageBlock, /const liveHref = isSnapshotBacked \? targetedSnapshotHref\(snapshotHref, snapshotTarget\) : ''/);
  assert.match(astImageBlock, /data-loom-interactive-snapshot-mode=\{liveHref \? 'click-to-load' : 'preview-link'\}/);
  assert.match(astImageBlock, /className="loom-snapshot-load"/);
  assert.match(astImageBlock, /data-loom-snapshot-href=\{liveHref\}/);
  assert.match(astImageBlock, /Click to load interactive/);
});
...
test('snapshot route supports targeted embed mode without rendering the whole page inline', () => {
  ...
  assert.match(snapshotPage, /CSS\.escape\(target\)/);
  assert.match(snapshotPage, /data-loom-snapshot-target-active/);
  assert.match(snapshotPage, /overflow: hidden !important/);
  assert.match(snapshotPage, /pointer-events: none !important/);
  assert.match(snapshotPage, /pointer-events: auto !important/);
